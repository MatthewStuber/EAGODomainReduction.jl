...
function STD_Linear_RR!(X::Vector{MCInterval{Float64}},opt,UBD::Float64)
    ...
    # stores outputs
    X[:] = [MCInterval(l[i],u[i]) for i=1:opt[1].numVar]
    return true
end
...
function Imp_Linear_RR!(X::Vector{Interval{Float64}},opt, UBD::Float64)
    ...
    f_mc::SMCg{np,Interval{Float64},Float64} = opt[1].solver.ImplicitOpts.f(x_mc[end],p_SMC)
    c::Vector{SMCg{np,Interval{Float64},Float64}} = opt[1].solver.ImplicitOpts.g(x_mc[end],p_SMC)
...
function Imp_Linear_RR!(X::Vector{Interval{Float64}},opt, UBD::Float64)
    ...
    f_mc::SMCg{np,MCInterval{Float64},Float64} = opt[1].solver.ImplicitOpts.f(x_mc[end],p_SMC)
    c::Vector{SMCg{np,MCInterval{Float64},Float64}} = opt[1].solver.ImplicitOpts.g(x_mc[end],p_SMC)
    ...
    X[(nx+1):(nx+np)] = [MCInterval(l[i],u[i]) for i=1:np]
    return true
end
